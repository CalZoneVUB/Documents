...
De werklast voor de vierde iteratie bepalen we opnieuw op basis van onze ervaring opgebouwd uit de voorbije iteraties. Uit de Time tracking vanop onze teamwebsite halen we volgende gegevens:
\begin{eqnarray}
	T_{persoon, 1ste\text{ }iteratie} &= 82u \\
	T_{persoon, 2de\text{ }iteratie} &= 57u \\
	T_{persoon, 3de\text{ }iteratie} &= 68u
\end{eqnarray}
Rekening houdend met het feit dat de vierde iteratie dichtbij het einde van het academiejaar ligt en er dus vrij veel teamleden ook veel werk hebben aan hun bachelorproef schatten we een werklast van:
\begin{equation*}
	T_{persoon, 4de\text{ }iteratie} = 45u
\end{equation*} 
In totaal, voor een team van 7 personen wordt dit:
\begin{equation*}
	T_{persoon} \approx 315u
\end{equation*}
Later zal dit getal vergeleken wordt met de gegevens beschikbaar uit de time tracking (zie sectie \ref{sec:BudgetControle}).
